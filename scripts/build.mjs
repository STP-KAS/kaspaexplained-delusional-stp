...
for(const name of ['app.mjs','network-diagram.mjs','models.mjs','app.css','money-app.mjs','money-models.mjs','coordination.mjs','coordination.css','network-diagram.css','mechanism-diagrams.mjs','mechanism-diagrams.css','flow-diagrams.mjs','flow-diagrams.css','wallet-holdings.mjs','installed-wallets.mjs','kachat.mjs','doors.mjs','door-ask.mjs','community.mjs','playground-testnet.mjs'])await copyFile(`src/${name}`,`${output}/assets/${name}`);
{
  ...
  await writeFile(`${output}/assets/door-intel.json`,JSON.stringify({
    doors:Object.values(doors).map(door=>({id:door.id,title:door.title,intel:door.intel,overview:strip(door.body)})),
    claims:claims.filter(c=>c.summary),
    notes:await readFile('content/master-notes.md','utf8'),
  }));
}
if(!standalone){
...
await writeFile(`${output}/.nojekyll`,'');
{
  ...
  if(source){
    await mkdir(join(output,'kachat','app'),{recursive:true});
    await cp(source,join(output,'kachat','app'),{recursive:true});
    console.log(`Attached KaChat Desktop from ${source} at /kachat/app/`);
  }else{
